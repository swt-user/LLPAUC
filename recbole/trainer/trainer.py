...
from recbole.trainer.optimizer import MinMax
...
class Trainer_MinMax(AbstractTrainer):
    ...
    def __init__(self, config, model):
        super(Trainer_MinMax, self).__init__(config, model)
        with open('./recbole/trainer/optim_hyper.json') as f:
            self.optim_hparams=json.load(f)
        self.all_counter=0
        ...
        saved_model_file = "{}-{}-{}-{}.pth".format(self.config["model"], get_local_time(),self.config['dataset_name'],self.config['type'])
        self.saved_model_file = os.path.join(self.checkpoint_dir, saved_model_file)
        self.weight_decay = config["weight_decay"]
        self.weight_dict=collections.defaultdict(float)
        self.start_epoch = 0
        self.cur_step = 0
    ...
    def _build_optimizer(self, **kwargs):
        ...
        return optimizer
    def _init_weight_dict(self,train_data):
        init_value=(self.model.config['neg_sample']*math.exp(math.log(self.model.config['inner']+1)/self.model.config['lam1']))**(-self.model.config['lam1']/self.model.config['lam2'])
        iter_data = (
            train_data
        )
        for batch_idx, interaction in enumerate(iter_data):
            ...
            for k in range(len(uid)):
                u=uid[k].item()
                self.weight_dict[u]=init_value

    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False,weight_dict=None):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses = loss_func(interaction,self.weight_dict)
            ...
            # if self.clip_grad_norm:
            #     clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            # self.optimizer.step(pre=False, t=self.all_counter)
            # self.all_counter+=1
        return total_loss
    ...
    def _save_checkpoint(self, epoch, verbose=True, **kwargs):
        ...
        # torch.save(self.model.state_dict(),'adressa_mf.pth')
        if verbose:
            self.logger.info(
    ...
    def fit(
        ...
        valid_step = 0
        self._init_weight_dict(train_data)
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            print('a',self.model.a.item(),'b',self.model.b.item(),'gamma',self.model.gamma.item(),'theta_a',self.model.theta_a.item(),'theta_b',self.model.theta_b.item(),'sp',self.model.sp.item(),'sn',self.model.sn.item())
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        try:
            ...
            self.if_gan=False
        if self.if_gan:
            self.gan_optimizer=self._build_optimizer()
    def _build_optimizer(self, **kwargs):
        r"""Init the Optimizer
    ...
    def _init_weight_dict(self,train_data):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            for k in range(len(uid)):
                u=uid[k].item()
                self.weight_dict[u]=init_value
    def _train_epoch_gan(self,train_data, epoch_idx, loss_func=None, show_progress=False):
        self.model.train()
        loss_func = loss_func or self.model.calculate_gan_loss
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            if self.clip_grad_norm:
                clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            scaler.step(self.gan_optimizer)
            scaler.update()
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if self.if_gan:
                training_start_time = time()
                train_loss_gan = self._train_epoch_gan(
                    train_data, epoch_idx, show_progress=show_progress
                )
                training_end_time = time()
                train_loss_output_gan = self._generate_train_loss_output(
                    epoch_idx, training_start_time, training_end_time, train_loss_gan
                )
                if verbose:
                    self.logger.info(train_loss_output_gan)
            self._add_train_loss_to_tensorboard(epoch_idx, train_loss)
            self.wandblogger.log_metrics(
                {"epoch": epoch_idx, "train_loss": train_loss, "train_step": epoch_idx},
                head="train",
